...
export class CkbDebuggerWasi {
  ...
  async runTransaction(
    ...
    return this.run(['--tx-file', txFile, '--script', script, '--max-cycles', maxCycles]);
  }
  ...
  async runBinary(
    ...
    return this.run(['--bin', binFile, '--script', script, '--max-cycles', maxCycles], {
      [binFile]: binFile,
    });
  }
  ...
  async runFast(
    ...
    return this.run(['--mode', 'fast', '--tx-file', txFile, '--script', script, '--max-cycles', maxCycles]);
  }
  ...
  async runWithStepLog(txFile: string, script: string = 'input.0.lock'): Promise<CkbDebuggerResult> {
    return this.run(['--tx-file', txFile, '--script', script, '--enable-steplog']);
  }
  ...
  async runGdbMode(
    ...
    return this.run(['--mode', 'gdb', '--tx-file', txFile, '--script', script, '--gdb-listen', gdbListen]);
  }
  ...
  async runWithProfile(
    ...
    return this.run(['--tx-file', txFile, '--script', script, '--pprof', pprofOutput]);
  }
}
